...
sub write_file {
    ...
    my $row_count = $output_hash_ref->{'count'};
    my $col_count = $output_hash_ref->{'countcols'};

    $worksheet->autofilter(0, 0, $col_count, $row_count);

    # Apply column formatting before population
    my $columnformat = $excel->add_format();
    $columnformat->set_text_wrap();
    $columnformat->set_align('left');
    $columnformat->set_align('top');

    $worksheet->set_column($a, $col_count, undef, $columnformat);

    my $rowformat = $excel->add_format();
    $rowformat->set_bold();        # Set BOLD text
    $rowformat->set_align('left'); # Set alignment
    $rowformat->set_align('top');

    # Process the header information
    foreach my $field ( @{$output_hash_ref->{'header'}} ) {
        $worksheet->write($row, $col, $field, $rowformat);
    ...
    foreach my $line_number ( sort keys %{$output_hash_ref->{'data'}} ) {
        ...
        foreach my $field ( @{$output_hash_ref->{'data'}->{$line_number}} ) {
            ...
            # Set the format for text-wrap
            $worksheet->write($row, $col, $field, $columnformat );
            $col++;
        }
    }
    ...
    foreach my $column_number ( sort keys %max_width ) {
        if ( $max_width{$column_number} > $max_field_width ) {
            $worksheet->set_column($column_number, $column_number, $max_field_width, $columnformat );
        }
        else {
            $worksheet->set_column($column_number, $column_number, $max_width{$column_number}, $columnformat );
        }
    }
    ...
    if ( -f $input_file && -r _ ) { # See if it is readable
        ...
        while ( my $inline = <$input_handle> ) {
            ...
            if ( $input_line == 0 ) {
                ...
                $output_hash_ref->{'countcols'} = scalar( @array );
            }
            else {
                # We have a line
                my @array =  split( $default_field_separator, $inline );
